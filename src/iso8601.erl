...
apply_months_offset(Datetime, AM) ->
    ...
    AY =
        case M + AM of
            %% Month and AM cancels each other, i.e 6 + -6
            0 -> ((Y - 1) * 12);
            MAM -> (Y * 12) + MAM
        end,
    Year = (AY div 12),
...
%"R2/P1Y3M22DT3H/2014-01-01T16:46:45Z"
parse_interval(TimeInterval) ->
    Tokens = string:tokens(TimeInterval, "/"),
    [R, S, E] =
        case string:substr(TimeInterval, 1, 1) of
            "R" ->
                [RS | StartEnd] = Tokens,
                [Start | Endd] = StartEnd,
        ...
        case is_datetime(S) of
            true ->
                case is_datetime(E) of
                    true ->
                        "Can't handle this yet (Date,Date)";
                    false ->
                        case is_duration(E) of
                            true -> [R, S, E];
                            false -> error(badarg)
                        end
                end;
